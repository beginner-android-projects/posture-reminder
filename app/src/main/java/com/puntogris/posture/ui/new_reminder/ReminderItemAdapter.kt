...
class ReminderItemAdapter(private val context: Context, private val clickListener: (ReminderUi)-> Unit):  RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(val item = items[position]){
            ...
            is ReminderUi.Item -> (holder as ReminderItemViewHolder).bind(item, clickListener)
        }
    }
    ...
    fun updateConfigData(reminder: Reminder){
        ...
        (items[6] as ReminderUi.Item).itemData.description = if (reminder.sound?.title.isNullOrBlank()) context.getString(R.string.disabled) else reminder.sound!!.title
